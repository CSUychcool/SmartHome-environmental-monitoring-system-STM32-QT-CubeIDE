# 智能家居/环境检测系统，编程环境为linux，用STM32F103C8T6采集传感器数据，QT作为上位机编写界面良好，两者之间通过USART串口通信，附加天气预报和deepseekAI小助手的功能
...
传感器采集信号输出到STM32，STM32采集到信号处理为数据，数据显示到OLED进行调试，数据通过串口发送json数据包到QT读取解析数据包
将阅读的信息进行实时显示，QT通过串口通信发送数据到下位机，下位机通过数据进行逻辑判断运行不同的代码逻辑
